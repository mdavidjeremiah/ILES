...
export function DashboardPage({ stats, placements, logs, evaluations, setActivePage, onSelectPlacement }) {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Dashboard"
        title="Internship Control Center"
        subtitle="Monitor placements, logbook submissions, visits, and evaluations."
        action={
          <button className="primary-button" type="button" onClick={() => setActivePage('logbook')}>
            <Plus size={18} />
            New Log
          </button>
        }
      ...
      <section className="dashboard-grid">
        <article className="panel wide">
          <div className="panel-header">
            ...
            <button type="button" onClick={() => setActivePage('placements')}>
              View all
            </button>
          </div>
            ...
            {placements.slice(0, 4).map((placement) => (
              <button key={placement.id} className="placement-row" type="button" onClick={() => onSelectPlacement(placement)}>
                <div>
                  ...
                  <small>
                    {placement.company_name} - {placement.department_code}
                  </small>
                </div>
        ...
        <article className="panel">
          ...
          <div className="stack-list">
            {recentLogs.map((log) => (
              <button key={log.id} type="button" onClick={() => setActivePage('logbook')}>
        ...
        <article className="panel">
          <div className="panel-header">
            ...
            <button type="button" onClick={() => setActivePage('evaluations')}>
              Open
            </button>
          </div>
