...
export function PlacementDetail({ placement, logs, evaluations, documents, visits, onClose }) {
    ...
    <Modal title="Placement Details" onClose={onClose}>
      <div className="detail-layout">
        ...
        <div className="detail-grid">
          <span>
            <BriefcaseBusiness size={16} /> Company <strong>{placement.company_name}</strong>
          </span>
          <span>
            <Users size={16} /> Department <strong>{placement.department_code}</strong>
          </span>
          <span>
            <CalendarDays size={16} /> Start <strong>{formatDate(placement.start_date)}</strong>
          </span>
          <span>
            <CalendarDays size={16} /> End <strong>{formatDate(placement.end_date)}</strong>
          </span>
        </div>
          ...
          <article>
            ...
            {placementEvaluations.map((evaluation) => (
              <span key={evaluation.id}>
                {evaluation.period} - {evaluation.score}
              </span>
            ))}
          </article>
          <article>
            ...
            {placementDocuments.map((document) => (
              <span key={document.id}>
                {document.title} - {labelize(document.status)}
              </span>
            ))}
          </article>
          <article>
            ...
            {placementVisits.map((visit) => (
              <span key={visit.id}>
                {formatDate(visit.scheduled_for)} - {labelize(visit.status)}
              </span>
            ))}
          </article>
        </div>
