...
export function EvaluationForm({ placements, onSubmit, onClose, saving }) {
  const [form, setForm] = useState({
    placement: placements[0]?.id || '',
    evaluator_name: '',
    evaluator_role: 'academic',
    period: 'Midterm',
    professionalism: 4,
    technical_skill: 4,
    communication: 4,
  ...
  return (
    ...
    >
      ...
      <label>
        Period
        <input value={form.period} onChange={(event) => update('period', event.target.value)} required />
      </label>
      <label>
        Status
